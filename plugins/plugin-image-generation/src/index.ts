...
import { type Action, type HandlerCallback, type IAgentRuntime, type Memory, ModelClass, type Plugin, type State } from '@elizaos/core';
import { generateImage } from '@elizaos/core';
import fs from 'fs';
import path from 'path';
import { validateImageGenConfig } from './environment.ts';
...
export function saveBase64Image(base64Data: string, filename: string): string {
  ...
  // Create a Uint8Array view of the buffer and save the file
  const uint8View = new Uint8Array(imageBuffer.buffer, imageBuffer.byteOffset, imageBuffer.byteLength);
  fs.writeFileSync(filepath, uint8View);
...
Ensure that your prompt is detailed, vivid, and incorporates all the elements mentioned above while staying true to the content and the specified style. LIMIT the image prompt 50 words or less. \n\nWrite a prompt. Only include the prompt and nothing else.`;
    ...
    if (images.success && images.data && images.data.length > 0) {
      ...
      for (let i = 0;i < images.data.length;i++) {
        ...
        elizaLogger.log(`Generated caption for image ${i + 1}:`, '...');
        ...
        }, [{ attachment: filepath, name: `${filename}.png` }]);
      }
    } else {
      elizaLogger.error('Image generation failed or returned no data.');
    }
  },
  examples: [[{ user: '{{user1}}', content: { text: 'Generate an image of a cat' } }, {
    user: '{{agentName}}',
