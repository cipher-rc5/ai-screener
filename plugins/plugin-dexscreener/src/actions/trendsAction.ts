import { type Action, elizaLogger, getEmbeddingZeroVector, type HandlerCallback, type IAgentRuntime, type Memory, type State } from '@elizaos/core';
...
const createTokenMemory = async (runtime: IAgentRuntime, _message: Memory, formattedOutput: string) => {
  const memory: Memory = {
...
export const latestTokensTemplate =
  `Determine if this is a request for latest tokens. If it is one of the specified situations, perform the corresponding action:
...
export const latestBoostedTemplate =
...
export const topBoostedTemplate =
  `Determine if this is a request for top boosted tokens. If it is one of the specified situations, perform the corresponding action:
